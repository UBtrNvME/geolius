...
class GeolocationService:
    ...
    def __init__(
        ...
        """
        self.city_db_path = (
            Path(city_db_path) if city_db_path else settings.get_city_db_path()
        )
        self.asn_db_path = (
            Path(asn_db_path) if asn_db_path else settings.get_asn_db_path()
        )
    ...
    def initialize(self) -> None:
        ...
        Raises ExternalApiError if the required City database is not found.
        """
    ...
    def _parse_city_response(
        ...
        # Get timezone
        timezone: str | None = (
            city_response.location.time_zone
            if city_response.location.time_zone
            else None
        )
    ...
    async def get_geolocation(self, ip_address: IPvAnyAddress) -> IpGeolocationResponse:
        ...
        if self._city_reader is None:
            raise ExternalApiError(
                "City database reader not initialized. Service may not have been properly started.",
                status_code=500,
            )
